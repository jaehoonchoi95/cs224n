...
\newcommand{\hmwkAuthorName}{Jaehoon Choi} % Your name
...
\begin{shaded}
\begin{answer}
Since the cross-entropy loss uses the one-hot vector y, only a single entry corresponding to a real outer word $\hat{y_{0}}$\, is counted, which is the same as the naive-softmax loss.
\end{answer}
\end{shaded}
...
\small %
\begin{align*}
    {\partial \over \partial v_{c}} J(\theta) &=  - {\partial \over \partial v_{c}} \log {\exp(u_o^T v_c) \over \sum_{w=1}^v \exp(v_w \cdot v_c) }
    = - {\partial \over \partial v_{c}} \log \exp(u_o^T  v_c) + {\partial \over \partial v_{c}} \log \sum_{w=1}^v \exp(v_w^T \cdot v_c) \\
    &= -u_o + {1 \over \sum_{w=1}^v \exp(u_w^T \cdot v_c)} \cdot  {\partial \over \partial v_{c}} \sum_{x=1}^v \exp(v_x^T v_c) 
    = -u_o + {1 \over \sum_{w=1}^v \exp(u_w^T  v_c)} \cdot 	\left( \sum_{x=1}^v \exp(u_x^T  v_c) \cdot {\partial \over \partial v_{c}} u_x^T  v_c\right)\\
    &=  -u_o + {1 \over \sum_{w=1}^v \exp(v_w^T  v_c)} \cdot \left(\sum_{x=1}^v \exp(u_x^T  v_c) \cdot u_x \right) = -u_0 + \sum_{x=1}^v (P(x | c) \cdot u_x) \\
    &=  -u_0 + U^T(\hat{y})= U^T(\hat{y} - y) 
\end{align*}
}
\item When $y$ and $\hat{y}$ match, the gradient becomes zero.
\item $v_c := v_c - \alpha \cdot (-u_0 + U^T(\hat{y}))$ $u_o$ represents the vector of the actual observed word in the context, anchoring the current word vector $v_c$ to the correct contextual usage. $U^T\hat{y}$ is the sum of all word vectors weighted by their predicted probabilities, representing the model's expected context. Subtracting $U^T\hat{y}$ from $v_c$ during the gradient update shifts $v_c$ towards $u_o$and away from potentially incorrect predictions, refining the word's contextual embedding.
\item In most cases, L2 normalization will not take away any useful information. However, it can cause a loss of useful information because it removes length information from the vectors (by making the total length equal to 1).  For example, for a hint given in a problem, even though the two vectors are not the same, this information is lost and normalized to the same vector.
\end{enumerate}
...
\small %
\begin{align*}
    {\partial \over \partial u_{w}} \bm J_{\text{naive-softmax}}(\bm v_c, o, \bm U) &=  - {\partial \over \partial u_w } \log {\exp(u_o^T v_c) \over \sum _{w=1}^v \exp (u_w v_c)} \\
    &= - {\partial \over \partial u_{w}} \log \exp(u_o^T  v_c) + {\partial \over \partial u_{w}} \log \sum_{w=1}^v \exp(v_w^T \cdot v_c) \\
    \textbf{If    } w = o, \\
    &= -v_c + {\exp(u_o^T v_c) \over \sum _{w=1}^v \exp (u_w v_c)} \cdot v_c \\
    &= v_c(p(o|c) - 1) = (\hat{y} - y)^T v_c \\
    \textbf{If    } w \neq o, \\
    &= 0 + {\exp(u_o^T v_c) \over \sum _{w=1}^v \exp (u_w v_c)} \cdot v_c \\
    & = v_c(p(o|c) - 0) = (\hat{y} - y)^T v_c
\end{align*}
\end{answer}
\end{shaded}
...
\small %
\begin{align*}
\nabla_{\bm U}  \bm J_{\text{naive-softmax}}(\bm v_c, o, \bm U) &= 	\left[ \frac{\partial \bm J(\bm v_c, o, \bm U)}{\partial \bm u_1}, \frac{\partial \bm J(\bm v_c, o, \bm U)}{\partial \bm u_2}, \cdots, \frac{\partial \bm J(\bm v_c, o, \bm U)}{\partial \bm u_{|\text{Vocab}|}}\right] \\
&= [(\hat{y_1}-y_1)v_c, (\hat{y_2}-y_2)v_c, \cdots (\hat{y}_{|\text{Vocab}|} - y_{|\text{Vocab}|})v_c] \;\;\;\;  ({\partial \over \partial u_{w}} \bm J(\theta) = (\hat{y} - y)^T v_c) \\
&= (\hat{y} - y)^T v_c
\end{align*}
...
\begin{answer}
\small %
${\partial \over \partial x} f(x) = \begin{cases}
        \alpha   \;\;\;\;\; \textbf{if} \;\; \alpha > 1\\
        1 \;\;\;\;\; \textbf{if} \;\; \alpha < 1
\end{cases}$
\end{answer}
\end{shaded}
...
\small %
\begin{align*}
{\partial \over \partial x} \sigma(x) &= {e^x(e^x + 1) - e^x(e^x) \over (e^x + 1)^2} \;\;\;\;\;\; (\text{By Quotient rule})\\
&= {e^x \over (e^x + 1)^2} = {e^x \over e^x + 1} \cdot {1 \over (e^x + 1)} \\
&= \sigma(x) (1 - \sigma(x))
\end{align*}
\end{answer}
\end{shaded}
...
\end{align*}
\item
\text{As shown in the above calculation, it is possible to simplify the calculation of the gradient by calculating}\\
\text{ $(1-\sigma(u_{ws}^T v_c))v_c$ This can be expressed as $1 - \sigma(\bm{U}_{o, \{w_1, \dots, w_K\}}^T v_c)$}
\item
\text{Negative sampling can be computed with lightweight operations on k negative samples, unlike naive softmax, } \\
\text{which requires traversing every word and performing inner product and exp operations.}
\end{enumerate}
\end{answer}
\end{shaded}
...
\begin{shaded}
\begin{answer}
$\bm J_{\text{neg-sample}}(\bm v_c, o, \bm U) = -\log(\sigma(\bm{u_o}^\top \bm{v_c})) - \sum_{\substack{1\le j \le k \\ w_j = w_s}} \log(\sigma(-\bm{u_{w_j}}^\top \bm{v_c})) - \sum_{\substack{1\le j \le k \\ w_i \neq w_s}} \log(\sigma(-\bm{u_{w_j}}^\top \bm{v_c}))$ \\[5pt]
\text{Splitting the expression into two sums eliminates the case $w_i \neq w_s$, following expression is derived} \\[5pt]
$\sum_{\substack{1\le j \le k \\ w_i \neq w_s}} (1-\sigma(u_{ws}^T v_c))v_c$
\end{answer}
\end{shaded}
...
\begin{answer}
    ...
    \item ${\frac{\partial \bm J_{\textrm{skip-gram}}(\bm v_c, w_{t-m},\ldots w_{t+m}, \bm U)} {\partial \bm v_w}} = 0$
\end{answer}
\end{shaded}
